...
#st.image("https://media.tenor.com/ZN76SLfmQtwAAAAd/human-error.gif", use_column_width=True)
st.write(f"<a href='{https://media.tenor.com/ZN76SLfmQtwAAAAd/human-error.gif}' target='_blank'><img src='{https://media.tenor.com/ZN76SLfmQtwAAAAd/human-error.gif}' width='200'></a>", unsafe_allow_html=True)
...
def adicionar_mensagem_erro():
    ...
    if len(st.session_state['erros']) >= 4:
        st.session_state['erros'].pop(2)  # Remove a mensagem mais antiga se já tiver 4 mensagens
    ...
    if st.button("Enviar senha para o controle"):
        if senha_digitada == senha_correta:
            ...
            for i in range(1, 6):  # 5 mensagens de sucesso
                    ...
                    time.sleep(4)  # Intervalo de 4 segundos entre as mensagens
...
if st.session_state.get('normalizacao_completa', False):
    if st.button("Próxima etapa", key="next_step"):
        st.empty()
    st.image("https://nuclear-power.com/wp-content/uploads/2020/05/reactor-steady-min.gif", use_column_width=True)
    st.write("<h1 style='text-align: center;'>MENSAGEM AQUI</h1>", unsafe_allow_html=True)
